...
import { Shield, Package, CreditCard, Ticket, Code, Trash2 } from "lucide-react";
...
export default function Admin() {
  ...
  const deleteItem = useMutation({
    mutationFn: async (id: number) => {
      const res = await fetch(api.items.delete.path.replace(':id', id.toString()), {
    ...
    onSuccess: () => {
      ...
      toast({ title: "ITEM DELETED", className: "bg-black border-primary text-primary" });
    },
    onError: () => {
              ...
              {items?.map((item: any) => (
                <div key={item.id} className="p-2 border border-white/10 text-sm flex justify-between items-center">
                  <div>
                    ...
                    <span className="text-muted-foreground ml-2 text-xs">{item.price} CR</span>
                  </div>
                  <button onClick={() => deleteItem.mutate(item.id)} className="text-destructive hover:text-destructive/80 transition-colors" data-testid="button-delete-item" title="Delete item">
